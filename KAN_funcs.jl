...
# define a stucture (type) that defines what data a SimpleKAN can hold
mutable struct SimpleKAN{N,M}
    num_input_nodes::N
    num_output_nodes::N
    poly_degree::N
    weights::M
...
# Define SimpleKAN constructor. This function helps build an object of the previously 
# defined SimpleKAN type.
# Example usage: model = SimpleKAN(3=>5,3) -> define architecture with input 
# dimensionality of 3 and 5 neurons per input. Use polynomial degree 3.
# To evaluate an output use model(x).
function SimpleKAN(node_config::Pair;poly_degree::Int=5,init=rand)
    ...
    return SimpleKAN(input_nodes,output_nodes,poly_degree,weights)
end

# define how the output of a layer is computed
function (l::SimpleKAN)(data::AbstractMatrix)
